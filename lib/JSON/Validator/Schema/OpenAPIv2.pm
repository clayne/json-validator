...
sub validate_response {
  ...
  return E '/', [status => 404]
    unless $method_path_status->[0] and $method_path_status->[1];

  my $parameters = $self->_build_res_parameters(@$method_path_status);
  my $status     = $method_path_status->[2] || 200;
  return E {
    details => [status => 501],
    message => qq(No response rule for "$status".)
    }
    unless $parameters;
...
sub version_from_class {
  ...
  $self->{data}{info}{version} = $class->VERSION if $class->VERSION;
  return $self;
}
